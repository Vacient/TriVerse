...
/* ---------- class / room-type catalogs (plan page pickers) ---------- */

/* Flight class choices: mult scales the base fare of every flight option. */
export const FLIGHT_CLASSES = [
  { id: "economy", label: "Economy", mult: 1 },
  { id: "premium-economy", label: "Premium Economy", mult: 1.35 },
  { id: "business", label: "Business", mult: 2.2 },
  { id: "first", label: "First Class", mult: 3.4 },
];

/* Room type choices: mult scales the nightly rate; bed config follows the room. */
export const ROOM_TYPES = [
  { id: "standard", label: "Standard Room", mult: 0.85, beds: "2 Single Beds", capacity: 2 },
  { id: "deluxe", label: "Deluxe Room", mult: 1, beds: "1 Double Bed", capacity: 2 },
  { id: "suite", label: "Suite", mult: 1.6, beds: "1 King Bed", capacity: 3 },
];
...
export function generatePlan(input) {
  ...
  // Fare lookup by origin airport code; pairs without catalogued pricing get a
  // stable, deterministic fare derived from the route id (works for any origin).
  const basePrice =
    dest.flightPrice[origin.code] ??
    90 + (hashString(dest.id + origin.id) % 121);
  const rnd = mulberry32(hashString(JSON.stringify(input)));

  // Primary/secondary airport: value & premium fly the primary (e.g. HND),
  // budget services fly the secondary (e.g. NRT). Single-airport cities reuse dest.code.
  const airports = dest.airports || [dest.code];
  const primaryAirport = airports[0];
  const secondaryAirport = airports[airports.length - 1];
  ...
  const hotelOptions = dest.hotels.map((h) => ({
    id: `h-${h.tier}`,
    ...h,
    ...ROOM_PROFILE[h.tier] || ROOM_PROFILE.boutique,
    nights,
    rooms: Math.ceil(travelers / 2),
    total: h.nightly * nights * Math.ceil(travelers / 2),
  }));

  // --- Budget fit scoring: recomputed from the exact budget the user typed ---
  const flightShare = budget * 0.45;
  const hotelShare = budget * 0.55;
  flightOptions.forEach((f) => {
    f.fitsBudget = Math.round(f.price * travelers) <= flightShare;
  });
  hotelOptions.forEach((h) => {
    h.fitsBudget = h.total <= hotelShare;
  });
  // Recommended hotel: the best tier that still fits the budget; fall back to cheapest.
  const fitsHotels = hotelOptions.filter((h) => h.fitsBudget);
  const recommendedHotel = fitsHotels.length
    ? fitsHotels[fitsHotels.length - 1]
    : hotelOptions[0];
  recommendedHotel.recommended = true;
  // Recommended flight: best value when it fits, otherwise the budget option.
  const recommendedFlight =
    flightOptions.find((f) => f.id === "f-value" && f.fitsBudget) ||
    flightOptions.find((f) => f.fitsBudget) ||
    flightOptions[0];
  recommendedFlight.recommended = true;
...
export function finalizePlan(draft, { flightIndex = 1, hotelIndex = 1 } = {}) {
  ...
  const departMin = 6 * 60 + 30; // 06:30 outbound
  ...
  // Balanced round-trip legs: same aircraft/duration in both directions,
  // price split 55/45 with a symmetrical return schedule.
  const returnDepartMin = 16 * 60; // 16:00 return
  const returnArriveMin = returnDepartMin + flight.durationMin + 60;
  const outboundPrice = Math.round(flight.price * 0.55);
  const returnPrice = flight.price - outboundPrice;
  ...
  for (let d = 0; d < days; d++) {
    ...
    if (isFirst) {
      items.push({
        ...
        desc: `Arrive at ${flight.airport} Airport (${dest.city}). Proceed through immigration and baggage claim.`,
        category: "TRANSIT",
        cost: 0,
        duration: 60,
        tags: [],
      ...
      items.push({
        ...
        desc: `Private car transfer from ${flight.airport} to ${hotel.name}, ${hotel.area}.`,
        category: "TRANSIT",
        cost: transfer.cost,
        duration: 45,
        tags: [],
    ...
    // Meal-aware scheduling: each food declares when it belongs (breakfast /
    // lunch / dinner / any) so breakfast dishes never land in a PM slot.
    const lunchPool = dest.foods.filter((f) => f.meal !== "breakfast" && f.meal !== "dinner");
    const lunch = weightedPick(lunchPool.length ? lunchPool : dest.foods, rnd, prefs, used);
    items.push({
      id: uid(),
      time: 12 * 60,
      title: lunch.name,
    ...
    if (!isFirst) {
      const baseMorning = [
        ...dest.attractions,
        ...dest.foods.filter((f) => f.cost <= 8 && f.meal === "breakfast"),
      ];
    ...
    // No evening dinner on the departure day — the return flight leaves at 16:00.
    if (!isLast) {
      const dinnerPool = dest.foods.filter((f) => f.meal !== "breakfast" && f.meal !== "lunch");
      const dinner = weightedPick(dinnerPool.length ? dinnerPool : dest.foods, rnd, prefs, used);
      items.push({
        id: uid(),
        time: 18 * 60 + 30,
        title: dinner.name,
    ...
    if (isLast) {
      items.push({
        id: uid(),
        time: returnDepartMin - 2 * 60,
        title: "Transfer to Airport",
        desc: `Head to ${flight.airport} with your bags — check-in opens 2h before departure.`,
        category: "TRANSIT",
      ...
      items.push({
        id: uid(),
        time: returnDepartMin,
        title: "Return Flight",
        desc: `Depart ${flight.airport} for ${origin.code}. ${flightH}h ${flightM}m, arrival ${origin.city}.`,
        category: "TRANSIT",
        cost: 0,
        duration: flight.durationMin,
        tags: [],
        scene: "plane",
        flightInfo: `${flight.airline.name} ${flight.airline.prefix} ${flight.number + 1} • ${flight.layovers ? "1 stop" : "Direct"} • ${flight.airport} → ${origin.code}`,
        flight: true,
      });
    }
    ...
    flight: {
      ...
      legs: {
        ...
        return: {
          airport: flight.airport,
          cabin: flight.cabin,
          departMin: returnDepartMin,
          arriveMin: returnArriveMin,
          depart: timeStr(returnDepartMin),
          arrive: timeStr(returnArriveMin),
          price: returnPrice,
        },
      },
    },
...
export function recomputeBudget(trip) {
  ...
  s.flights = (
    trip.flight.legs
      ? trip.flight.legs.outbound.price + trip.flight.legs.return.price
      : trip.flight.price
  ) * t;
...
export function buildRecommendations(trip) {
  ...
  if (paid.length) {
    ...
    const best = paid[0];
    const activitySave = best.it.cost * trip.travelers;
    recs.push({
      id: "free-attraction",
      icon: "ticket",
      title: "Free attraction swap",
      detail: `Replace "${best.it.title}" with a free viewpoint that matches your style — save ${money(activitySave)}.`,
      save: activitySave,
      available: true,
      target: best.it.id,
    });
...
export function agentSteps(trip) {
  return [
    ...
    },
    {
      ...
      detail: `Found ${trip.flightOptions?.length || 3} potential routes from ${trip.origin.code} to ${trip.flight.legs ? trip.flight.legs.outbound.airport : trip.dest.code}.`,
    },
    {
